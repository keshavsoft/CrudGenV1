...
dotenv.config();
const CommonSort = "SubTable";

let StartFunc = ({ inTablesCollection, inTo }) => {
    let LocalTypeName = "SubTable/restClients/GetEndPoints";
    let LocalTo = inTo;
...
const LocalFuncWriteToHome = ({ inFrom, inTo }) => {
    let LocalFileData = `GET http://localhost:${inFrom}/${CommonSubTable}`;
...
const LocalFuncWriteToDataOnly = ({ inFrom, inTo }) => {
    ...
    let LocalFileData = `GET http://localhost:${inFrom}/${CommonSubTable}/${LocalEndPoint}`;
